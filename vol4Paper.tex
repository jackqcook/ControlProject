...
a scheduler selects the nearest operating point to the current state estimate at each time step, applying the corresponding gains. This allows us to maintain close to optimal performance across a broad range of flight dynamics.
...
\end{align*}
where $J_x, J_y, J_z$ are the principal moments of inertia, $l$ is the rotor arm length, and $c_\tau$ is the torque coefficient. This formula comes from the angular momentum cross-product formula, which we need to use because of the effects each rotor has on the other moments of rotation.
...
\]
via central finite differences. This is accomplished as follows: For all 12 columns 
of the 12x12 Jacobian matrix, we populate the column by the formula 

\[
J_{:,i} = \frac{f(x_{eq} + \epsilon e_i, u_{eq}) - f(x_{eq} - \epsilon e_i, u_{eq})}{2\epsilon}
\]
and B is computed similarly with the other argument of $f$.

This yields the continuous-time linear model
\[
...
each linearization using a forward Euler approximation with time step $\Delta t$:
\[
...
before simulation begins, so that during the simulation the scheduler simply performs a nearest-neighbor lookup to select the appropriate $A_d$, $B_d$, $K$, and $L$ at each time step.
...
\bibitem{}
Dul, Franciszek, Piotr Lichota, and Artur Rusowicz. "Generalized linear quadratic control for a full tracking problem in aviation." Sensors 20.10 (2020): 2955.

\bibitem{}
Doyle, John C. "Guaranteed margins for LQG regulators." (1978).
